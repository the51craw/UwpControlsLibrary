...
namespace UwpControlsLibrary
{
    ...
    public partial class Oscillator
    {
        ...
        /// <summary>
        /// IsSub true transposes one octave down.
        /// </summary>
        public bool IsSub;
        ...
        {
            if (synthesis != null && key != 0xff)
            {
                keyboardAdjustedFrequency = finetunedFrequency * synthesis.NoteKeys.NoteFrequency[key] / 440f;
                if (Keyboard)
                {
        ...
        public Oscillator(Synthesis synthesis, int poly)
        {
        ...
        public void Init()
        {
            PitchEnvelope = new PitchEnvelope(synthesis, this);
            //WaveData = new double[mainPage.SampleCount * 2];
            Adsr = new ADSR(synthesis);
            Keyboard = true;
            frequency = 440;
            lfoFrequency = 5;
            FineTune = 0.0f;
        ...
        /// </summary>
        /// <param name="key"></param>
        public void ReinitializeOscillator(int channel, byte key, byte velocity)
        {
            Key = key;
            KeyOff = false;
            KeyOn = false;
            ...
            StepSize = (1 + PitchEnvelope.Value * PitchEnvelope.PitchEnvPitch) * FrequencyInUse * Math.PI * 2 / synthesis.SampleRate;
            if (IsSub)
            {
                StepSize /= 2;
            }
            ...
            {
                AM_Modulator.ReinitializeOscillator(channel, key, velocity);
            }
            if (FM_Modulator != null)
            {
                FM_Modulator.ReinitializeOscillator(channel, key, velocity);
            }
            if (XM_Modulator != null)
            {
                XM_Modulator.ReinitializeOscillator(channel, key, velocity);
            }
        }
        ...
        /// <returns></returns>
        public void GenerateAudioData(uint requestedNumberOfSamples, LfoAdvance lfoAdvance)
        {
            //randomValue = 0;
            ...
            SetStepSize(this); // Sets modulators step size.
            StepSize = synthesis.PitchBend[MidiChannel] * 
                (1 + PitchEnvelope.Value * PitchEnvelope.PitchEnvPitch) * 
                FrequencyInUse * Math.PI * 2 / synthesis.SampleRate;
            if (IsSub)
            {
                StepSize /= 2;
            }
            ...
            {
                lowSineWaveCompensation = synthesis.EarCompensation.KeyToGain(key);
            }
            ...
            for (int i = 0; i < requestedNumberOfSamples * 2; i++)
            {
                ...
                if (lfoAdvance == LfoAdvance.ADVANCE)
                {
                ...
                //if (Terminate > -1)
                //{
                //    if (CurrentSample < 0.00001 && CurrentSample > -0.00001)
                //    {
                //        Adsr.AdsrRelease(true);
                //        Terminated = true;
                //    }
                //}
            ...
            {
                Adsr.Advance();
                synthesis.allowGuiUpdates = true;
            }
        }
        ...
        /// </summary>
        /// <param name="speed">0.0 - 1.0</param>
        /// <param name="depth">0.0 - 1.0</param>
        public void SetChorus(double speed, double depth)
        {
        ...
        {
            oscillator.StepSize = synthesis.PitchBend[incomingMidiChannel] * 
                (1 + oscillator.PitchEnvelope.Value * oscillator.PitchEnvelope.PitchEnvPitch) * 
                oscillator.FrequencyInUse * Math.PI * 2 / synthesis.SampleRate;
