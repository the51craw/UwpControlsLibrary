...
namespace UwpControlsLibrary
{
    ...
    public class KeyDispatcher
    {
        ...
        public KeyDispatcher(Synthesis synthesis, int polyphony)
        {
            this.synthesis = synthesis;
            Polyphony = polyphony;
            Key = new int[Polyphony];
            ...
            if (!pedalHold)
            {
                for (int poly = 0; poly < Polyphony; poly++)
                {
                    if (!IsPressed[poly])
                    {
                        for (int osc = 0; osc < 12; osc++)
                        {
                            synthesis.NoteKeys.KeyOff((byte)Key[poly], synthesis.Oscillators[poly][osc].MidiChannel);
                        }
                    }
                }
            }
