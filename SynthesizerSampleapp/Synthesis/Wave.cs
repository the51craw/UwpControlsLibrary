...
namespace UwpControlsLibrary
{
    ...
    public class Wave
    {
        ...
        public Wave(Synthesis synthesis, StorageFolder filesLocation)
        {
        ...
        private async Task MakeAudioFileInputNode(StorageFile file, int key, double frequencyFactor)
        {
            ...
            CreateAudioFileInputNodeResult fileInputResult = 
                await synthesis.FrameServer.theAudioGraph.CreateFileInputNodeAsync(file);
            ...
            else
            {
                ...
                Keys[key].AddOutgoingConnection(synthesis.FrameServer.Mixer);
                Keys[key].Stop();
            }
        }
        ...
        private async Task GetWaveFile(StorageFile file)
        {
            ...
            CreateAudioFileInputNodeResult fileInputResult = await synthesis.FrameServer.theAudioGraph.CreateFileInputNodeAsync(file);
            if (fileInputResult.Status != AudioFileNodeCreationStatus.Success)
            {
                fileParsingResults.Add("The file " + file.Name + " could not be opened by the AudioGraph.");
            }
