    ...
    public partial class Oscillator : ControlBase
    {
        ...
        public double MakeModulatedWave(Channel channel, OscillatorUsage usage)
        {
            ...
            if (WaveForm == WAVEFORM.SQUARE)
            {
                ...
                {
                    if (Get_XM_Sensitivity(this) > 0)
                    {
                        Phase = Math.PI + Get_XM_Sensitivity(this) / 125.0 * XM_Modulator.MakeModulatedWave(channel, OscillatorUsage.MODULATION) * Math.PI;
                    }
                }
            ...
            else if (WaveForm == WAVEFORM.NOISE)
            {
                if (AM_Modulator != null)
                {
                    sample = MakeNoiseWave() * (0.5 + Get_AM_Sensitivity(this) * AM_Modulator.MakeModulatedWave(channel, OscillatorUsage.MODULATION) / 2) * Math.PI;
